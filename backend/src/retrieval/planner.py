...
- Use the language that best matches the supplied library context.
- Keep each query concise, ideally 5-14 words.
...
LOCAL_EXCLUSION_PATTERN = re.compile(
    r"\b(?:without|excluding|except|not)\s+([^\W_]+(?:[-'][^\W_]+)*(?:\s+[^\W_]+(?:[-'][^\W_]+)*){0,2})",
    re.IGNORECASE,
)
        ...
        *,
        ...
        try:
            ...
            for item in raw_suggestions:
                ...
                if suggestion and len(suggestion) <= 120:
        ...
        *,
        ...
        for asset in context_assets[:12]:
            for term in list(asset.get("tags") or [])[:6]:
                ...
                if cleaned and cleaned not in context_terms:
                    context_terms.append(cleaned)
        concepts = list(
            dict.fromkeys(
                cleaned
                for term in list(library_summary.get("top_concepts") or [])
                if (cleaned := re.sub(r"\s+", " ", str(term or "")).strip())
            )
        )
        places = list(
            dict.fromkeys(
                ...
                if (cleaned := re.sub(r"\s+", " ", str(place or "")).strip())
            )
        )
        ...
        for memory in memories[:4]:
            ...
            label = re.sub(r"\s+", " ", str(memory.get("label") or "")).strip()
    ...
    def _extract_required_terms(
        *,
        ...
        excluded_tokens = {
            ...
            for token in re.findall(r"[^\W_]+", phrase.lower())
        }
        required_terms: list[str] = []
